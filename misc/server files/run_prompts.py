...
# Global Directories
##############################
# Change these two variables to set the base input and output paths.
INPUT_DIR = "/home/ra37qax/"
OUTPUT_DIR = "/home/ra37qax/"
...
def load_model_and_tokenizer(model_name: str):
    ...
    # Set padding token and side for the Llama model
        ...
        try:
            ...
            for prompt in batch_prompts:
                ...
                while not success and retries < 3:
                    try:
                        small_batch = tokenizer([prompt], return_tensors="pt", padding=True)
                        small_batch = {k: v.to("cuda") for k, v in small_batch.items()}
                        with torch.no_grad():
                            out = model.generate(**small_batch, max_new_tokens=max_new_tokens)
                        outputs.extend(out)
                        del small_batch, out
                        success = True
                    except torch.cuda.OutOfMemoryError:
...
def classify_adherence_to_guide(df: pd.DataFrame, batch_size=4, checkpoint_interval=1000) -> pd.DataFrame:
    if "llm_adherence_score" not in df.columns:
        df["llm_adherence_score"] = 0.0

    # Use OUTPUT_DIR for checkpoint saving.
    checkpoint_path = os.path.join(OUTPUT_DIR, "adherence_checkpoint.csv")
...
def classify_personal_sharing(df: pd.DataFrame, batch_size=4):
    ...
    for conversation_id, turns in df.groupby("conversation_id"):
        ...
        for i in range(2, len(turns)):
            ...
            if (target_turn["words"] is None or 
                target_turn["SpeakerTurn"] == turns["SpeakerTurn"].max() or 
                len(target_turn["words"].split()) < 5):
                continue
            total_prompts += 1
    ...
    for conversation_id, turns in df.groupby("conversation_id"):
        ...
        for i in range(2, len(turns)):
            row = turns.iloc[i]
            if (row["words"] is None or 
                row["SpeakerTurn"] == turns["SpeakerTurn"].max() or 
                len(row["words"].split()) < 5):
                continue
            ...
            if len(batch_prompts) >= batch_size:
                responses = batch_generate(batch_prompts, max_new_tokens=128, batch_size=batch_size, verbose=False)
    ...
    if batch_prompts:
        responses = batch_generate(batch_prompts, max_new_tokens=128, batch_size=len(batch_prompts), verbose=False)
        for meta, response in zip(results_meta, responses):
            response_text = response.strip()
            if "Answer:" in response_text:
                response_text = response_text.split("Answer:")[-1].strip()
...
def classify_speech_acts(df: pd.DataFrame, batch_size=4) -> pd.DataFrame:
    for col in ["speech_acts_result", "express_appreciation", "express_affirmation", "open_invitation", "specific_invitation"]:
    ...
    for conversation_id in conversation_ids:
        turns = grouped.get_group(conversation_id).sort_values("SpeakerTurn").reset_index(drop=True)
        for i in range(2, len(turns)):
            target_turn = turns.iloc[i]
            if (target_turn["words"] is None or 
                target_turn["SpeakerTurn"] == turns["SpeakerTurn"].max() or 
...
- **Open invitation to participate:** Invites anyone in the group to speak.
- **Specific invitation to participate:** Directly invites a particular person to speak.
...
Answer:"""
                ...
                idx = df.index[
                    (df["conversation_id"] == conversation_id) & (df["SpeakerTurn"] == target_turn["SpeakerTurn"])
                ].tolist()
                if len(idx) == 1:
                    row_idx = idx[0]
...
def main():
    ...
    tokenizer, model = load_model_and_tokenizer("nvidia/Llama-3.1-Nemotron-70B-Instruct-HF")

    # --- Adherence-to-Guide Classification ---
    adherence_input_path = os.path.join(INPUT_DIR, "adherence_results_threshold_0.4.csv")
    ...
    adherence_output_csv = os.path.join(OUTPUT_DIR, "adherence_results_classified.csv")
    adherence_output_pkl = os.path.join(OUTPUT_DIR, "adherence_results_classified.pkl")
    ...
    data_input_path = os.path.join(INPUT_DIR, "data_nv-embed_processed_output.pkl")
    ...
    speech_acts_csv_path = os.path.join(OUTPUT_DIR, "speech_acts_classification.csv")
    speech_acts_pkl_path = os.path.join(OUTPUT_DIR, "speech_acts_classification.pkl")
    ...
    output_csv = os.path.join(OUTPUT_DIR, "speech_acts_personal_sharing_classification.csv")
    output_pkl = os.path.join(OUTPUT_DIR, "speech_acts_personal_sharing_classification.pkl")
